...
let jtfr = {
    tFind: [
        "KriegerBank",
        "https://github.com/Algersoft/kriegerbank",
        "Kriegerrand",
        "KREGR",
        "kriegerrand-service"
    ],
...
function setDarkMode(dark) {
    ...
    if (tmode === 'dark') {
        ...
        dmswitch.firstChild.classList.remove('fa-free-code-camp');
        dmswitch.firstChild.classList.add('fa-dumpster-fire');
        settings.set('darkmode', true);
        dmswitch.firstChild.dataset.icon = 'dumpsterfire';
    } else {
        thtml.classList.remove('dark');
        dmswitch.setAttribute('title', 'Swith to dark mode');
        dmswitch.firstChild.classList.remove('fa-dumpster-fire');
        dmswitch.firstChild.classList.add('fa-free-code-camp');
        settings.set('darkmode', false);
        dmswitch.firstChild.dataset.icon = 'code';
    }
}
...
function handleAddressBook() {
    ...
    addressBookButtonSave.addEventListener('click', () => {
        ...
        if (addressValue.length > 102) paymentIdValue.value = '';
...
function handleWalletClose() {
    overviewWalletCloseButton.addEventListener('click', (event) => {
        ...
        let htmlStr = '<div class="div-save-main" style="text-align: center;padding:1rem;"><i class="fas fa-atom fa-pulse"></i><span style="padding:0px 10px;">Saving &amp; closing your wallet...</span></div>';
...
function handleSendTransfer() {
    ...
    sendButtonSend.addEventListener('click', () => {
        ...
        if (recipientAddress.length > 102) {
            paymentId = '';
...
ipcRenderer.on('cleanup', () => {
    ...
    let htmlText = 'Terminating KriegerBank...';
    if (wsession.get('loadedWalletAddress') !== '') {
        htmlText = 'Saving &amp; closing your wallet...';
    }

    let htmlStr = `<div class="div-save-main" style="text-align: center;padding:1rem;"><i class="fas fa-atom fa-pulse"></i><span style="padding:0px 10px;">${htmlText}</span></div>`;
